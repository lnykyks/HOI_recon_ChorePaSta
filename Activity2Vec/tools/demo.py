...
class Activity2Vec():
    ...
    def inference_copy(self, image_path, image=None):
        if image is None:
            ori_image = im_read(image_path)
            alpha_image = cv2.cvtColor(ori_image, cv2.COLOR_BGR2RGB)
        else:
        ...
        if pose is None:
            self.logger.info('[Activity2Vec] no pose result for {:s}'.format(image_path))
            vis = ori_image
            pasta = self.vis_tool.draw_copy(vis, None, None, None, None, None, None)
            return ori_image, None, pasta
        else:
            try:
                pasta_image, annos = self.pasta_model.preprocess(ori_image, pose['result'])
                ...
                p_pasta = p_pasta[score_filter]
                p_verb = p_verb[score_filter]
                pasta = self.vis_tool.draw_copy(vis, bboxes, keypoints, scores, p_pasta, p_verb, human_ids)
                ...
                annos_cpu['p_pasta'] = p_pasta
                annos_cpu['p_verb'] = p_verb
                return ori_image, annos_cpu, pasta
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, choices=['image', 'video', 'lin_chore'], default='image', # 加一个给chore跑的，读文件的代码重新设计
...
if __name__ == '__main__':
    ...
    if args.mode == 'image':
        ...
        print('len: {}'.format(len(image_list))) # linycs add
        for image_path in tqdm(image_list):
            ...
            if len(args.output) > 0:
                print('detected ouput dir') # linycs add
    ...
    elif args.mode == 'lin_chore':
        import json
        chore_dir = args.input
        '''
            behave_test_images/Date03_Sub03_backpack_back/t0003.000
            其中
                behave_test_images是chore的数据集根目录，
                Date03_Sub03_backpack_back是动作类别，
                t0003.000是动作序列
        '''
        sequences = os.listdir(chore_dir)
        for seq in sequences:
            seq_path = os.path.join(chore_dir, seq)
            steps = os.listdir(seq_path)
            print('sequence: {}'.format(seq))
            for step in tqdm(steps, total=len(steps)):
                step_path = os.path.join(seq_path, step)
                if not os.path.isdir(step_path):
                    continue
                # images = os.listdir(step_path)
                image_path = os.path.join(step_path, 'k1.color.jpg')
                if not os.path.exists(image_path):
                    # image_path = image_path.replace('.jpg', '.png') # 可以再试一下png存在否
                    print('image_path: {} not exist'.format(image_path))
                json_path = os.path.join(step_path, 'k1.pasta.json')
                if os.path.exists(json_path):
                    # print('json exists')
                    continue

                ori_image, annos, pasta = a2v.inference_copy(image_path)
                json.dump(pasta, open(json_path, 'w'))
                # print('{} dumped'.format(json_path))
            #     break
            # break
