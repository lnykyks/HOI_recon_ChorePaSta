...
class DataPaths:
    ...
    def get_image_paths_seq(seq, tid=1, check_occlusion=False):
        ...
        for img_file in image_files:
            # print(img_file) # linycs add
            mask_file = img_file.replace('.color.jpg', '.obj_rend_mask.png') # linycs edit
            full_mask_file = img_file.replace('.color.jpg', '.obj_rend_full.jpg').replace('_images', '_fullmask') # linycs edit
            if not isfile(mask_file) or not isfile(full_mask_file):
                # print('not file') # linycs add
                # print(f'{mask_file}: {isfile(mask_file)}')
                # print(f'{full_mask_file}: {isfile(full_mask_file)}')
                continue
        ...
        print( f'loaded {len(valid_files)}/{len(image_files)}')
        return valid_files
