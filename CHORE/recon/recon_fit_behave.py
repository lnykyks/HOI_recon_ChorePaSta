...
class ReconFitterBehave(ReconFitterBase):
    def fit_recon(self, args):
        ...
        for i, data in enumerate(loop):
            ...
            # !  每个轮数变量设定了不同优化过程的迭代次数
            # 比如 beta是最开始优化的，达到iter_for_beta次后开始优化pose，迭代pose次后开始优化fine-tune
            smpl, scale = self.optimize_smpl(
                smpl, betas_dict, 
                max_iter=150, 
                # # ! down: linycs add
                # iter_for_kpts=1, iter_for_pose=1, iter_for_betas=1, 
                # steps_per_iter=1, max_iter=50
            ) # coco data
            ...
            data_dict = {
                ...
                'pasta': data['pasta'], # linycs add
            }

            smpl, obj_R, obj_t = self.optimize_smpl_object(
                generator.model, data_dict, 
                steps_per_iter=1, obj_iter=1, joint_iter=1, # linycs add
            ) # cuda ordinal bug's entrance
    ...
    def optimize_smpl_object(self, model, data_dict, obj_iter=20, joint_iter=10, steps_per_iter=10):
        ...
        # iterations=1
        # iter_for_obj=1
        # max_iter, prev_loss = 20, 300. # linycs add
        # iter_for_sil = 20
        # steps_per_iter=2
        ...
        for it in loop:
            ...
            for i in range(steps_per_iter):
                loss_dict = self.forward_step(model, smpl_split, data_dict, obj_R, obj_t, obj_s, phase) # cuda ordinal bug's entrance
    ...
    def forward_step(self, model, smpl, data_dict, obj_R, obj_t, obj_s, phase):
        ...
        if phase == 'sil':
            ...
            if phase == 'joint':
                ...
                # 这个很奇怪，没有返回值。但这里面有划分了part。linycs add

                # prevent interpenetration
                pen_loss = self.compute_collision_loss(
                    smpl_verts, smpl.faces,
                    R, obj_t, obj_s, 
                    data_dict['pasta'], # linycs add
                ) # cuda ordinal bug's entrance
    ...
    def optimize_smpl(self, smpl, data_dict, iter_for_betas=10, iter_for_pose=10,
        ...
        # max_iter, prev_loss = 1, 300. # for debug, linycs add, next line as well
