...
import pytorch3d # linycs add
# import pasta_lin # linycs add
...
import json
human_parts = json.load(open('/mnt/e/lnyky2/projects/HOI_recon/PHOSA/models/meshes/person_labels_palm.json', 'r'))['labels'] # linycs add
...
class ReconFitterBase:
    def __init__(self, seq_folder,
        ...
        # for collision loss
        self.scan_faces = torch.tensor(self.scan.f).long().to(self.device) # N*3
        sigma = 0.5
        point2plane = False
        max_collisions = 8
    ...
    def save_outputs(self, smpl, obj_R, obj_t, traindata_paths, save_name, test_id, obj_s=None):
        '''
            linycs add
            save the ply of human+object together
        '''
        ...
        for v, p, r, s, t in zip(
            verts, 
            obj_files, 
            obj_R.detach().cpu().numpy(), 
            obj_s.detach().cpu().numpy(), 
            ...
            smpl_obj_verts = np.vstack([verts_np[0], verts[0]])
            smpl_obj_faces = np.vstack([faces_np, self.scan.f+verts_np.shape[1]]) # bias of object face index
            s_o = Mesh(v=smpl_obj_verts, f=smpl_obj_faces)
            s_o.write_ply(p.replace('object', 'whole'))
    ...
    def smpl_obj_collision(self, smpl_verts, smpl_faces, obj_verts, obj_faces):
        ...
        with torch.no_grad():
            ...
            collision_idxs = self.search_tree(triangles) # cuda ordinal bug's entrance, 实际上 triangles还是顶点集
        pen_loss = torch.mean(self.pen_distance(triangles, collision_idxs))
        return pen_loss

    def compute_collision_loss(self, smpl_verts, smpl_faces, obj_R, obj_t, obj_s, pasta=None):
        """
        ...
        # # smpl_verts and verts' shape == [1,N,3]
        # pen_loss = self.smpl_obj_collision(smpl_verts, smpl_faces, verts, self.scan_faces) # cuda ordinal bug's entrance
        # return pen_loss

        '''
            person_mesh = pytorch3d.structures.Meshes(verts=smpl_verts, faces=smpl_faces.view(1,-1,3))
            obj_pc = pytorch3d.structures.Pointclouds(points=verts)
            pen_loss = pytorch3d.loss.point_mesh_distance.point_mesh_face_distance(person_mesh, obj_pc)
        '''

        # self.part_labels: (6890, ), value represent part index
        pasta_parts = {
            # 'hip':
            'lhand': 2, # part index in self.part_labels
            'rhand': 7,
            'lfoot': 1,
            'rfoot': 6,
        } # part state to be checked
        if pasta is not None:
            check_parts = list(pasta_parts.keys())
            for p in check_parts:
                if p not in pasta['0'].keys():
                    continue
                state = pasta['0'][p][0]
                if state not in self.states[p[1:]]:
                    pasta_parts.pop(p) # remove part that has no action
            # left pasta contains action
        
        if len(pasta_parts) > 0: # exists part(s) with action
            ...
            object_mesh = trimesh.Trimesh(
                ...
                faces=self.scan_faces.detach().cpu().numpy(), 
                process=False, 
                validate=False, 
                maintain_order = True, 
            )
            person_mesh = trimesh.Trimesh(
                vertices=smpl_verts[0].detach().cpu().numpy(), 
                faces=smpl_faces.detach().cpu().numpy(), 
                process=False, 
                validate=False, 
                maintain_order = True, 
            )
            cm = trimesh.collision.CollisionManager()
            cm.add_object('object', object_mesh)
            cm.add_object('person', person_mesh)
            result, contact_data = cm.in_collision_internal(return_data=True)
            ...
            for p in pasta_parts:
                ...
                p_loss, _ = pytorch3d.loss.chamfer_distance(verts, part_verts.unsqueeze(0))
                pen_losses.append(p_loss)
