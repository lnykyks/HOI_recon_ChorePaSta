    ...
    def train_model(self, epochs):
        ...
        for epoch in tqdm(range(start, epochs)):
            ...
            for batch in loop:
                ...
                if iteration_duration > self.ck_period * 60:  # eve model every X min and at start
                    ...
                    if self.rank == 0 and batch_count > 0:
                        ...
                        self.writer.add_scalar('training loss steps', 
                            sum_loss / batch_count,
                            batch_count + epoch * len(train_data_loader)
                        )
                        self.writer.add_scalars('train losses steps', 
                            self.format_separate_loss(separate_losses,batch_count),
                            batch_count + epoch * len(train_data_loader)
                        )
            ...
            if self.rank == 0:
                ...
                self.writer.add_scalars('train losses', 
                    self.format_separate_loss(separate_losses,len(train_data_loader)), 
                    epoch
                )
